...
import { geminiService, ClipSuggestion } from "../services/gemini.service";
import { getStorageService, getStorage } from "../storage";
import { env } from "../config/env";
import { JOB_STATUS, PROCESSING_STEPS } from "../config/constants";
import { logger } from "../utils/logger";
import { JobResult, JobProgress } from "../shared/types/job";
  ...
  private async processWithGemini(job: any): Promise<void> {
    ...
    const clips = await clipService.generateClipsFromSuggestions(
      ...
      jobDir,
      {
        ...
        framingMode: job.options.framingMode ?? 'crop', // Default to crop mode
      },
      (clipIndex, percent) => {
    ...
    const finalClips = clips.map((clip, index) => {
      ...
      return {
        ...
        score: clip.score,
        thumbnailUrl: `${env.BACKEND_URL}/outputs/${job.id}/thumbnails/${clip.id}.jpg`,
        videoUrl: `${env.BACKEND_URL}/outputs/${job.id}/clips/${clip.id}.mp4`,
