...
import { Scene } from "../shared/types/clip";
...
export interface ClipGenerationOptions {
  ...
  framingMode?: 'crop' | 'fit'; // 'crop': center crop (default), 'fit': fit with letterboxing
}
...
export class ClipService {
    ...
    const clipPromises = scenes.map(async (scene, i) => {
      ...
      try {
        const clip = await this.generateClip(
          ...
          options.framingMode ?? 'crop', // Default to crop mode
  ...
  private async generateClip(
    ...
    framingMode: 'crop' | 'fit' = 'crop',
    onProgress?: (percent: number) => void
  ): Promise<GeneratedClip> {
    const clipPath = path.join(outputDir, "clips", `${clipId}.mp4`);
    const thumbnailPath = path.join(outputDir, "thumbnails", `${clipId}.jpg`);

    const targetWidth = VIDEO_CONFIG.defaults.resolution.width;
    const targetHeight = VIDEO_CONFIG.defaults.resolution.height;

    // Build video filters
    const filters: string[] = [];

    if (framingMode === 'fit') {
      // FIT MODE: Scale video to fit within 1080x1920 frame with letterboxing
      // This preserves the entire video content without cropping
      filters.push(
        `scale=w=${targetWidth}:h=${targetHeight}:force_original_aspect_ratio=decrease`,
        `pad=${targetWidth}:${targetHeight}:(ow-iw)/2:(oh-ih)/2:black`
      );
    } else {
      // CROP MODE (default): Center crop to 9:16 vertical format
