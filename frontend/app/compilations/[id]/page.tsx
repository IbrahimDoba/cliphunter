...
import {
  ...
  NarrationStyle,
} from '@/lib/shared/types/compilation';
...
export default function CompilationPage() {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const response = await fetch(`${(() => process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:4000")()}/api/compilations/${compilationId}`);
  ...
  const handleStartRender = async () => {
    ...
    try {
      ...
      if (compilation?.script) {
        await fetch(`${(() => process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:4000")()}/api/compilations/${compilationId}/script`, {
          method: 'PUT',
      ...
      // Start render
      const response = await fetch(`${(() => process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:4000")()}/api/compilations/${compilationId}/render`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRegenerateScript = async () => {
    ...
    try {
      const response = await fetch(`${(() => process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:4000")()}/api/compilations/${compilationId}/regenerate-script`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
